...
class OrderPaidFragment : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(R.layout.fragment_order_paid, container, false)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        (activity as MainActivity).mSearchListener = { query ->
            isSearching = true
            orderPaidViewModel.fetchOrderPaid("", query).run { showLoading() }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        orderPaidViewModel.fetchOrderPaid(mCurrentPage.toString(), "")
        orderPaidViewModel.fetchSumMoney()
        orderPaidViewModel.fetchNumberOrder()




        orderPaidViewModel.moneyLiveData.observe(viewLifecycleOwner, Observer {
            if(it.length > 4){
        ...
        orderPaidViewModel.numberOrderLiveData.observe(viewLifecycleOwner, Observer {
        ...
        mAdapter.mPaymentListener = { order ->
            ...
            val alertDialog: AlertDialog =
                android.app.AlertDialog.Builder(requireContext()) //set icon
                    .setTitle("Bạn muốn hủy đơn này ?") //set message
                    ...
                    .setPositiveButton("Cập nhật",
                        DialogInterface.OnClickListener { dialogInterface, i -> //set what would happen when positive button is clicked
                            orderPaidViewModel.updateOrder(order.id).run {
                                showLoading()
                            }
                        }) //set negative button
